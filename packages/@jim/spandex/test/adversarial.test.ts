...
Deno.test('Adversarial - All patterns', async (t) => {
	await t.step('Concentric rectangles (maximize overlaps)', () => {
		...
		for (let i = 0; i < 50; i++) {
			...
			index.insert(rect.make(i, i, size, size), `value${i}`);
	...
	await t.step('Diagonal sweep (maximize edge cases)', () => {
		...
		for (let i = 0; i < 100; i++) {
			index.insert(rect.make(i, i, i + 19, i + 19), `value${i}`);
	...
	await t.step('Checkerboard (maximize decomposition)', () => {
		...
		for (let i = 0; i < 10; i++) {
			// Large block
			index.insert(rect.make(0, i * 20, 99, (i + 1) * 20 - 1), `block${i}`);
		}
		...
		for (let i = 0; i < 50; i++) {
			...
			index.insert(rect.make(col, row, col + 2, row + 2), `hole${i}`);
		}
	...
	await t.step('RStarTree under same patterns', () => {
		...
		for (let i = 0; i < 50; i++) {
			...
			index.insert(rect.make(i, i, size - 1, size - 1), `value${i}`);
	...
	await t.step('Growth pattern analysis', () => {
		...
		for (let n = 10; n <= 100; n += 10) {
			...
			for (let i = 0; i < n; i++) {
				...
				testIndex.insert(rect.make(i, i, size - 1, size - 1), `v${i}`);
			}
	...
	await t.step('Stress test with random overlaps', () => {
		...
		for (let i = 0; i < 200; i++) {
			...
			index.insert(rect.make(xmin, ymin, xmin + width - 1, ymin + height - 1), `random${i}`);
		}
