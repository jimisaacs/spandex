...
Deno.test('LazyPartitionedSpatialIndexImpl - Geometry', async (t) => {
	...
	await t.step('Empty index', async (context) => {
		const index = new LazyPartitionedSpatialIndexImpl<Record<string, string>>(
			() => new MortonLinearScanImpl(),
		);

		const rendered = render(() => index.query(), {});
	...
	await t.step('Fragment count validation - LWW decomposition', async (context) => {
		const index = new LazyPartitionedSpatialIndexImpl<Record<string, string>>(
			() => new MortonLinearScanImpl(),
		);
	...
	await t.step('Query bounds filtering', async (context) => {
		const index = new LazyPartitionedSpatialIndexImpl<Record<string, string>>(
			() => new MortonLinearScanImpl(),
		);
	...
	await t.step('Infinite rectangles - finite extent auto-detection', async (context) => {
		...
		index.set([0, 0, Infinity, Infinity], 'fontColor', 'blue');
		...
		// Debug: show rectangles that determined grid size
		const results = Array.from(index.query());
		console.log('\nRectangles in index:');
		for (const [bounds, attrs] of results) {
			console.log(`  [${bounds}] → ${JSON.stringify(attrs)}`);
		}
