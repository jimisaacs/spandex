...
Deno.test('A1 Edge Cases', async (t) => {
	...
	await t.step('Range Normalization', () => {
		...
		const [normalBounds] = Array.from(adapter.query())[0]!;
	...
	await t.step('Mixed Column and Row Ranges', () => {
		...
		assertEquals(col?.[0][1], r.negInf, 'Full column has infinite rows');
		assertEquals(col?.[0][3], r.posInf, 'Full column has infinite rows');
	});
