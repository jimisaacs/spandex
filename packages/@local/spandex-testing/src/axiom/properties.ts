...
export async function testPropertyAxioms(
	...
	await t.step('Value preservation', () => {
		const index = implementation();
		index.insert(rect.make(1, 1, 4, 4), 'test');
	...
	await t.step('Idempotency', () => {
		const index = implementation();
		const testRange = rect.make(1, 1, 4, 4);
	...
	await t.step('Invalid range rejection', () => {
		...
		let threw = false;
		try {
			index.insert(rect.make(1, 5, 2, 4), 'invalid-row'); // ymin=5 > ymax=4
		} catch (e) {
			threw = true;
		...
		threw = false;
		try {
			index.insert(rect.make(5, 1, 4, 2), 'invalid-col');
		} catch (e) {
			threw = true;
			assertIsError(e, Error);
		}
	...
	await t.step('Coordinate extremes', () => {
		...
		index.insert(rect.make(largeCoord, largeCoord, largeCoord + 99, largeCoord + 99), 'large');
		assertInvariants(index, 'large coordinates');

		const results = Array.from(
			index.query(rect.make(largeCoord + 50, largeCoord + 50, largeCoord + 59, largeCoord + 59)),
		);
		assertEquals(results.length, 1, 'query at large coordinates should find exactly 1 result');
		assertEquals(results[0][1], 'large');

		index.insert(rect.make(0, 0, 9, 9), 'small');
		assertInvariants(index, 'mixed coordinate scales');

		const smallResults = Array.from(index.query(rect.make(0, 0, 9, 9)));
	...
	await t.step('Query basic', () => {
		...
		index.insert(rect.make(1, 1, 4, 4), 'red');
		index.insert(rect.make(10, 10, 14, 14), 'blue');
		index.insert(rect.make(20, 20, 24, 24), 'green');
		assertInvariants(index, 'query setup');

		const results = Array.from(index.query(rect.make(3, 3, 11, 11)));
		...
		const emptyResults = Array.from(index.query(rect.make(100, 100, 109, 109)));
	...
	await t.step('Query without bounds returns all', () => {
		...
		index.insert(rect.make(0, 0, 4, 4), 'topleft');
		index.insert(rect.make(10, 10, 14, 14), 'center');
		index.insert(rect.make(20, 5, 24, 9), 'rightmid');
		index.insert(rect.make(5, 20, 9, 24), 'botmid');
	...
	await t.step('Query boundary precision', () => {
		...
		const emptyResults = Array.from(index.query(rect.make(0, 0, 9, 9)));
		assertEquals(emptyResults.length, 0, 'query on empty index returns empty');

		index.insert(rect.make(5, 5, 14, 14), 'center');
		index.insert(rect.make(20, 20, 29, 29), 'corner');
		assertInvariants(index, 'query setup');

		const allResults = Array.from(index.query(rect.make()));
		assertEquals(allResults.length, 2, 'infinite query returns all entries');

		const exactMatch = Array.from(index.query(rect.make(5, 5, 14, 14)));
		assertEquals(exactMatch.length, 1, 'exact match finds one range');
		assertEquals(exactMatch[0][1], 'center');

		const partialOverlap = Array.from(index.query(rect.make(10, 10, 24, 24)));
		assertEquals(partialOverlap.length, 2, 'partial overlap finds both ranges');

		const noOverlap = Array.from(index.query(rect.make(100, 100, 109, 109)));
	...
	await t.step('Query-insert consistency', () => {
		...
		const testCases = [
			{ range: rect.make(10, 10, 19, 19), value: 'center' },
			{ range: rect.make(0, 0, 99, 4), value: 'top-strip' },
			{ range: rect.make(0, 0, 4, 99), value: 'left-strip' },
			{ range: rect.make(1000, 1000, 1009, 1009), value: 'far' },
			{ range: rect.make(0, 0, 0, 0), value: 'single-cell' },
		];
	...
	await t.step('Query completeness', () => {
		...
		for (let i = 0; i < 10; i++) {
			index.insert(rect.make(i * 2, 0, i * 2 + 2, 19), `value${i}`);
		}

		assertInvariants(index, 'after 10 insertions');

		const results = Array.from(index.query(rect.make(0, 0, 19, 19)));
		...
		const subResults = Array.from(index.query(rect.make(5, 5, 14, 14)));
	...
	await t.step('Non-overlapping preservation', () => {
		...
		for (let i = 0; i < 5; i++) {
			for (let j = 0; j < 4; j++) {
				...
				index.insert(rect.make(baseCol, baseRow, baseCol + 49, baseRow + 49), value);
			}
		}
	...
	await t.step('Complex insertion sequence preserves invariants', () => {
		...
		const deterministicPatterns = [
			rect.make(0, 0, 4, 4),
			rect.make(10, 10, 14, 14),
			rect.make(2, 2, 7, 7),
			rect.make(5, 20, 9, 24),
			rect.make(15, 1, 19, 5),
			rect.make(3, 12, 8, 17),
			rect.make(12, 5, 17, 10),
			rect.make(7, 15, 13, 21),
			rect.make(18, 3, 23, 8),
			rect.make(1, 7, 6, 12),
		];
	...
	await t.step('Fragment count: small overlapping (50 ops → 63)', () => {
		const operations = Array.from({ length: 50 }, (_, i) => ({
			range: rect.make(i % 5, Math.floor(i / 3), (i % 5) + 2, Math.floor(i / 3) + 2),
			value: `s_${i}`,
		}));
	...
	await t.step('Fragment count: diagonal pattern (20 ops → 39)', () => {
		const operations = Array.from({ length: 20 }, (_, i) => ({
			range: rect.make(i * 2, i * 2, i * 2 + 4, i * 2 + 4),
			value: `d_${i}`,
		}));
	...
	await t.step('Fragment count: large overlapping (1250 ops → 1375)', () => {
		const operations = Array.from({ length: 1250 }, (_, i) => ({
			range: rect.make(i % 10, Math.floor(i / 5), (i % 10) + 4, Math.floor(i / 5) + 4),
			value: `overlap_${i}`,
		}));
